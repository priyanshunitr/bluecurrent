...
import { fetchMotorStatus, toggleMotorState, updateSchedules, unlinkMotor, renameMotor, fetchMyMotors } from '../services/api';
...
const MotorDetails = () => {
    ...
    const handleRename = async () => {
        ...
        if (trimmedName === motorData.nickname) {
        ...
        try {
            ...
            const nameExists = motors.some(m => 
                m.hexcode !== hexcode && 
                (m.nickname || "").toLowerCase() === trimmedName.toLowerCase()
            );

            if (nameExists) {
                showStatus('error', 'Conflict', `A motor named "${trimmedName}" already exists. Please choose a different name.`);
                setRenaming(false);
                return;
            }

            await renameMotor(hexcode, trimmedName);
            setMotorData(prev => ({ ...prev, nickname: trimmedName }));
            setShowRenameModal(false);
